...
Database: Neon PostgreSQL (psycopg2-binary — reliable, battle-tested driver)
Hosting: Render
"""
...
def get_db():
    """
    Open a psycopg2 connection to Neon.
    psycopg2 accepts the full Neon connection string (including ?sslmode=require)
    directly — no manual URL parsing or ssl_context needed.
...
# Run init_db at startup — full traceback shown in Render logs on failure
...
def account():
    ...
    try:
        ...
        user = cur.fetchone()
        cur.execute(
            "SELECT COUNT(*) as cnt FROM course_progress WHERE user_id = %s AND completed = TRUE",
            (user_id,)
        )
